...
# def save_index(index, output_path, metadata=None):
#     """
...
# def process_local(folder_path, output_path):
#     """
...
#         metadata.extend([{"source": str(file_path), "chunk": i} for i in range(len(chunks))])

#     embeddings = embed_texts(all_texts)
#     index = create_faiss_index(embeddings)
#     save_index(index, output_path, metadata)


# def process_s3(bucket, prefix, output_path):
#     """
...
#         chunks = split_text(text)
#         all_texts.extend(chunks)
#         metadata.extend([{"source": key, "chunk": i} for i in range(len(chunks))])
...
#     parser.add_argument("--input", help="Local path or S3 prefix to input documents")
...
#         process_s3(args.bucket, args.input, args.output)
